...
class State:
    ...
    def finish(self):
        ...
        if self.get_player_order() == 0: # no pass
            mine_count -= red_komi
            ...
            if mine_count >= prev_count and mine_count >= next_count:
                ...
                if mine_count >= next_count:
                    ...
                    return [-1, 0, 1]
        elif self.get_player_order() == 1: #  pass 1
    ...
    def next(self, action):
        ...
        state.mine_pieces = state.next_pieces   #當下輪到的顏色就是mine
...
def maxn(state, depth, bound):    
    if state.is_done() or depth == 0:
        if state.is_done():
            s = state.finish_reward()  # 使用修改後的獎勵分配
            ns = np.array(s)+1                      
            s = list(ns * Bound / sum(ns))  # 轉換為相對分數
