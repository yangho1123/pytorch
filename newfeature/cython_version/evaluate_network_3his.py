...
def update_best_player():
    copy('../model/250504/22layers/best_val.pt', '../model/250504/22layers/best.pt')
...
def evaluate_network(index, points_latest, points_best, no1_latest, no1_best):
    ...
    # 载入模型
    model_latest = torch.jit.load('../model/250504/22layers/best.pt').to(device)   #lab pv-mcts 1000場訓練後的
    model_best = torch.jit.load('../model/250504/22layers/best_val.pt').to(device) # 學弟maxn 2000場訓練後的
    # 評估模式
    # model_latest.eval()
    # model_best.eval()
