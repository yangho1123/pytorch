...
def apv_worker(model, in_queue, out_queue):
    model.eval()
    ...
    while True:
        ...
        if data == "STOP":
            break
        state, path = data
        policies, value = predict(model, state, path)
        out_queue.put((policies, value))
...
def pv_mcts_scores(model, state, temperature, add_noise=True, dirichlet_alpha=0.03):
    in_queue = mp.Queue()
    out_queue = mp.Queue()
    workers = [mp.Process(target=apv_worker, args=(model, in_queue, out_queue)) for _ in range(NUM_WORKERS)]
    for w in workers:
        w.start()
