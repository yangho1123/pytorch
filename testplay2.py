...
from game import random_action, maxn_action, maxn_actionnw, TEMPERATURE
import torch
...
if __name__ == '__main__': # 我方不等於state.mine_pieces
    ...
    win_count_blue = 0
    # print("無權重+無分組")
    print("6層1vs3層2、有權重有分組測時間")
    for i in range(game_count):
        start_time = time.time()
        print("==== game:", i+1, "====")
        if i%3 == 0:
            results, times = play([maxn6,maxn3,maxn3])   # 這局紅用maxn下法
        elif i%3 == 1:
            results, times = play([maxn3,maxn6,maxn3])   # 這局綠用maxn下法
        elif i%3 == 2:
            results, times = play([maxn3,maxn3,maxn6])   # 這局藍用maxn下法
        ...
        print("一場時間{:.2f}秒".format(time.time() - start_time) )
    # 計算每步的平均時間
    ...
    plt.xlabel('round')
    plt.ylabel('time(s)')
    plt.title('Maxn6 Average Time with group')
    plt.legend()
    plt.grid(True)
    plt.savefig('./plots/Maxn6_with_group.png')
