...
def predict(model, state, path): # 利用對偶網路做下一步的預測    
    ...
    for i in range(history_length + 1):  # +1 to include the current state
        if len(path) > i:
            ...
            full_state_tensor = torch.cat((state_tensor, player_channel.unsqueeze(0)), dim=0)
            board_states.append(full_state_tensor)
        else:
            # 如果没有历史状态，使用零张量
            zero_state = torch.zeros((4, 121))
            board_states.append(zero_state)
               
    # Stack all states to create a tensor of shape [8, 4, 11, 11]    
    x = torch.stack(board_states).float()   # Ensure float type for model processing
    x = x.view(1, 4 * (history_length + 1), 11, 11)            # Reshape to [1, 32, 11, 11]
    ...
    class node:
        ...
        def evaluate(self):
            ...
            if not self.child_nodes: # 當前node沒有子節點 -> expand
                ...
                policies, value = predict(model, self.state, path)  # 假設value是一介於-1~1之間的小數
                self.w += value      
                self.n += 1
                self.child_nodes = []
                ...
                self.w += value     # 上面兩種情況回傳的value都是一個小數，因此這邊的value應為一個小數(-1~1)
                self.n += 1
                return value
            
        def next_child_node(self): # select     w價值一定是一個數，而不是一個串列，只有finish才會傳回串列
            ...
            for child_node in self.child_nodes:
                pucb_values.append((child_node.w[player] / child_node.n if child_node.n else 0.0) +
                                   C_PUCT * child_node.p * sqrt(t) / (1 + child_node.n))    #假設我選了某一個子節點，那麼他以我的角度來說，價值多少
